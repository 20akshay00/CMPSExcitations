...
begin
	...
	function solve(solver::QuadratureSolver, kernel, λ, g, yi, yf)
	    ...
	    # solve the linear system (I - λKW)f = g
	    # K .* row_vector scales the COLUMNS of K (K_ij * w_j).
	    fs = (I - λ * (K .* transpose(vec(ws)))) \ gs
		return xs, fs
		
	    return function(x)
	        integral_term = dot(ws .* fs, kernel.(x, xs))
	        return λ * integral_term + g(x)
	    end
	end
	
	# f(x) - λ∫dy kernel(x, y) f(y) = g(x)
	function solve(solver::ModifiedQuadratureSolver, kernel, λ, g, yi, yf)
		(;weights, roots, N) = solver
		K = kernel.(roots, transpose(roots))
		gs = g.(roots)
	
		A = sum(K, dims=2)
		Δ = Diagonal(weights .* K .- A)
		f = (I + λ * (Δ - K .* transpose(weights))) \ gs
		return (x) -> λ * sum(kernel(x, roots) .* transpose(weights .* f)) + g(x)
	end
end

# ╔═╡ 30dfcaeb-6a69-458a-bc15-7c03d5d9e86b
let
	f = solve(
		QuadratureSolver(gausslegendre(60)), 
		(x, y) -> x * y,
		1.,
		x -> x,
		-1., 
	...
	xs = minimum(f[1]):0.1:maximum(f[1])
	#plot(xs, f, lab = "Numerical")
	scatter(f..., lab = "Numerical")
	plot!(xs, xs, lab = "Exact")
end

# ╔═╡ 1abf76cb-4256-4f6a-873c-4d2333f73a89
let
	f = solve(
		QuadratureSolver(gausslegendre(100)), 
		(x, y) -> min.(x, y),
		1.,
...
let
	f = solve(
		QuadratureSolver(gausslegendre(500)), 
		(x, y) -> abs.(x .- y),
		1/2,
		x -> exp(x),
		-1., 
...
manifest_format = "2.0"
project_hash = "724e2cc39655c31f0803573413d4ac4ab3052d77"
